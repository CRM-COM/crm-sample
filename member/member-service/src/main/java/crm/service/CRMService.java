...
//        .header(HttpHeaders.AUTHORIZATION, "Basic " + crmConfig.getKey())
  ...
  public CRMContactDetails getMember(String crmId) {
    return Optional.ofNullable(getContactDetailsFromCrm(crmId))
            .map(CRMContactDetailsResponse::getData).orElse(null);
  }

  private CRMContactDetailsResponse getContactDetailsFromCrm(String crmId) {
    return restTemplate.getForObject(crmConfig.getUrl() + "/contact_information/show?token=" + getToken() +
            "&contact_information_identifier=id=" + crmId, CRMContactDetailsResponse.class);
  }
}
